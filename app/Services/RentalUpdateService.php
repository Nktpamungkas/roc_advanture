...
{
    public function update(User $actor, Rental $rental, array $validated): Rental
    {
        return DB::transaction(function () use ($actor, $rental, $validated): Rental {
            ...
            $currentItems = $lockedRental->items->keyBy('inventory_unit_id');
            $currentUnitIds = $currentItems->keys()->map(fn ($id) => (int) $id)->all();
            ...
            if ($inactiveProducts !== []) {
                throw ValidationException::withMessages([
                    'inventory_unit_ids' => 'Produk nonaktif tidak bisa ditambahkan ke rental: '.implode(', ', $inactiveProducts).'.',
                ]);
            }
            ...
            $paidAmount = round((float) $lockedRental->paid_amount, 2);
            ...
            $this->syncRentalItemsAndUnits($lockedRental, $inventoryUnits, $currentItems, $totalDays);
    ...
    private function syncRentalItemsAndUnits(
        Rental $rental,
        Collection $inventoryUnits,
        Collection $currentItems,
        int $totalDays,
    ): void {
        $nextUnitIds = $inventoryUnits->pluck('id')->map(fn ($id) => (int) $id)->all();
        $currentUnitIds = $currentItems->keys()->map(fn ($id) => (int) $id)->all();

        $removedUnitIds = array_values(array_diff($currentUnitIds, $nextUnitIds));
        if ($removedUnitIds !== []) {
            foreach ($removedUnitIds as $removedUnitId) {
                $item = $currentItems->get($removedUnitId);

                if ($item !== null) {
                    InventoryUnit::query()
                        ->whereKey($removedUnitId)
        ...
        $rental->items()->createMany(
            $inventoryUnits
                ->map(function (InventoryUnit $inventoryUnit) use ($currentItems, $totalDays): array {
                    $currentItem = $currentItems->get($inventoryUnit->id);
                    $statusAtCheckout = $currentItem?->status_at_checkout;

                    if ($statusAtCheckout === null) {
                        $statusAtCheckout = in_array($inventoryUnit->status, [
                            InventoryUnitStatuses::READY_CLEAN,
                            InventoryUnitStatuses::READY_UNCLEAN,
                        ], true) ? $inventoryUnit->status : InventoryUnitStatuses::READY_CLEAN;
                    }
                    ...
                    return [
                        ...
                        'status_at_checkout' => $statusAtCheckout,
