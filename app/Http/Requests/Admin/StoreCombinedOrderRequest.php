    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            foreach ($saleItems as $index => $item) {
                ...
                if ((int) $item['qty'] > (int) $saleProduct->stock_qty) {
                    $validator->errors()->add("sale_items.$index.qty", sprintf('Stok "%s" tidak cukup. Tersedia %d.', $saleProduct->name, $saleProduct->stock_qty));
                }
            }

            if ($validator->errors()->hasAny(['starts_at', 'due_at', 'rental_days', 'inventory_unit_ids', 'sale_items', 'payment_method_config_id'])) {
                return;
            }
