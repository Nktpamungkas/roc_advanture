    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if (! $rental instanceof Rental) {
                $validator->errors()->add('starts_at', 'Transaksi rental tidak ditemukan.');

                return;
            }

            if (! in_array($rental->rental_status, [RentalStatuses::BOOKED, RentalStatuses::PICKED_UP, RentalStatuses::LATE], true)) {
                $validator->errors()->add('starts_at', 'Hanya rental aktif yang bisa diedit.');

                return;
            }

            $currentUnitIds = $rental->items()->pluck('inventory_unit_id')->map(fn ($id) => (int) $id)->all();
            ...
            if ($unavailableUnits !== []) {
                $validator->errors()->add(
                    'inventory_unit_ids',
                    'Unit inventaris berikut sudah tidak ready untuk dipakai di transaksi ini: '.implode(', ', $unavailableUnits).'.',
                );
            }

            $inactiveProducts = $inventoryUnits
                ->filter(function (InventoryUnit $inventoryUnit) use ($currentUnitIds): bool {
                    return ! in_array($inventoryUnit->id, $currentUnitIds, true) && ! (bool) $inventoryUnit->product?->active;
                })
                ->pluck('product.name')
                ->filter()
                ->unique()
            ...
            if ($inactiveProducts !== []) {
                $validator->errors()->add(
                    'inventory_unit_ids',
                    'Produk nonaktif tidak bisa ditambahkan ke rental: '.implode(', ', $inactiveProducts).'.',
                );
            }
            ...
            if ((float) $rental->paid_amount < $requiredDp && blank($this->input('dp_override_reason'))) {
                $validator->errors()->add(
                    'dp_override_reason',
